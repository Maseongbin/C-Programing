...
void Excersize_2_5(int solution, int answer)
{
    ...
    if (solution == SHOW)
    {
        printf("\n\n\n");
        printf("=================================================================\n");
        printf("관련 공식은 시간(h) = 거리(d) / 속력(v)입니다.\n");

        printf("이 사람이 처음 %0.1lf km를 갔을 때 걸린 시간은 %0.1lf km(갈 때의 이동거리) / %0.1lf km/h(갈 때의 속력) = %0.1lf h(걸린 시간)이다. \n", distance1, distance1, velocity1, time1);
        printf("이 사람이 다시 %0.1lf km를 왔을 때 걸린 시간은 %0.1lf km(올 때의 이동거리) / %0.1lf km/h(올 때의 속력) = %0.1lf h(걸린 시간)이다. \n", distance2, distance2, velocity2, time2);
        printf("점심시간 식사를 하는데 걸리는 시간은 %0.1lf이다.\n", time3);
        all_time = time1 + time2 + time3;
        printf("이 사람의 여행과정에서 총 걸린시간은 %0.1lf h(갈 때의 시간) + %0.1lf h(올 때의 시간) + %0.1lf h(식사 한 시간) = %0.1lf h이다. \n", time1, time2, time3, all_time);
        all_distance = distance1 + distance2;
        printf("이 사람의 여행과정에서 총 이동한 거리는 %0.1lf km(갈 때의 거리) + %0.1lf km(올 때의 거리) = %0.1lf km이다. \n", distance1, distance2, all_distance);
    ...
    if (answer == SHOW)
    {
        printf("=========================   정 답   =============================\n");
        printf("(a) 이 사람의 여행과정에서 평균 속력은 얼마인가? \n");
        printf("관련 공식은 평균 속력 = 총 이동 거리(나중 거리 - 처음 거리) / 총 걸린 시간(나중 시간 - 처음 시간)입니다.\n");
        ...
        printf("이 사람의 평균 속력은 %0.1lf km(총 이동 거리) / %0.1lf h(총 걸린 시간) = %0.1lf km/h(평균 속력) \n", all_distance, all_time, average_velocity);
        printf("이 사람의 여행과정에서 평균 속력: %0.1lf km/h \n", average_velocity);
        printf("=================================================================\n");
        printf("\n\n\n");
    }
    ...
    if (solution == SHOW)
    {
        ...
        printf("관련 공식은 평균 속도 = 변위 / 총 걸린 시간입니다.\n");
        displacement = distance1 - distance2;
        printf("이 사람이 %0.1lf km갔다가 다시 %0.1lf km 되돌아 왔기 때문에 변위는 %0.1lf km 이다.\n", distance1, distance2, displacement);
        all_time = time1 + time2 + time3;
        printf("이 사람의 여행과정에서 총 걸린시간은 %0.1lf h(갈 때의 시간)+ %0.1lf h(올 때의 시간) + %0.1lf h(식사 한 시간) = %0.1lf h이다. \n", time1, time2, time3, all_time);
    ...
    if (answer == SHOW)
    {
        ...
        printf("이 사람의 여행과정에서 평균 속도는 %0.1lf km(변위) / %0.1lf h(총 걸린 시간) = %0.1lf km/h(평균 속도) \n", displacement, all_time, average_speed);
